[![Build Status](https://travis-ci.org/damif94/revive.svg?branch=master)](https://travis-ci.org/damif94/revive)
...
```bash
go install github.com/damif94/revive@latest
```

or get a released executable from the [Releases](https://github.com/damif94/revive/releases) page.

You can install the main branch (including the last commit) with:
```bash
go install github.com/damif94/revive@master
```
...
```bash
docker run -v "$(pwd)":/var/<repository> ghcr.io/damif94/revive:v1.1.2-next -config /var/<repository>/revive.toml -formatter stylish ./var/kidle/...
```

- `-v` is for the volume  
- `ghcr.io/damif94/revive:v1.1.2-next ` is the image name and its version corresponding to `revive` command
...
- `-exclude [PATTERN]` - pattern for files/directories/packages to be excluded for linting. You can specify the files you want to exclude for linting either as package name (i.e. `github.com/damif94/revive`), list them as individual files (i.e. `file.go`), directories (i.e. `./foo/...`), or any combination of the three.
...
```shell
revive -config revive.toml -exclude file1.go -exclude file2.go -formatter friendly github.com/damif94/revive package/...
```
...
- The linter will analyze `github.com/damif94/revive` and the files in `package`
...
```shell
revive -config defaults.toml github.com/damif94/revive
```

This will use the configuration file `defaults.toml`, the `default` formatter, and will run linting over the `github.com/damif94/revive` package.

### Custom Configuration

```shell
revive -config config.toml -formatter friendly github.com/damif94/revive
```

This will use `config.toml`, the `friendly` formatter, and will run linting over the `github.com/damif94/revive` package.
...
import (
	"github.com/damif94/revive/cli"
	"github.com/damif94/revive/lint"
	"github.com/damif94/revive/revivelib"
)
